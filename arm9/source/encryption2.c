...
static u32 bswap_32bit2 (u32 in) {
	u8 a,b,c,d;
	a = (u8)((in >>  0) & 0xff);
	b = (u8)((in >>  8) & 0xff);
	c = (u8)((in >> 16) & 0xff);
...
void apply_keycode2 (u32 modulo) {
	u32 scratch[2];
	int i;
	modulo = modulo / sizeof(*keycode);

	crypt_64bit_up2 (&keycode[1]);
	crypt_64bit_up2 (&keycode[0]);
	toncset (scratch, 0, 8);

	for (i = 0; i < 0x12; i+=1) {
		keybuf[i] = keybuf[i] ^ bswap_32bit2 (keycode[i % modulo]);
	}
	for (i = 0; i < 0x412; i+=2) {
		crypt_64bit_up2 (scratch);
		keybuf[i]   = scratch[1];
...
void init_keycode2 (u32 idcode, u32 level, u32 modulo, int iCardDevice) {
	...
	if (level >= 1) apply_keycode2 (modulo);	// first apply (always)
	if (level >= 2) apply_keycode2 (modulo);	// second apply (optional)
	keycode[1] = keycode[1] * 2;
	keycode[2] = keycode[2] / 2;
	if (level >= 3) apply_keycode2 (modulo);	// third apply (optional)
}
